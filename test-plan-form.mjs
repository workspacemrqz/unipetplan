...
(async () => {
  ...
  try {
    ...
    const editButtons = await page.$$('a[href*="/admin/planos/editar"]');
    if (editButtons.length > 0) {
      ...
      // Aguardar um pouco para garantir que o React carregou tudo
      await new Promise(resolve => setTimeout(resolve, 2000));
      
      // Verificar o conteúdo do campo de texto do contrato
      const contractTextElement = await page.$('textarea');
      if (contractTextElement) {
        const contractText = await page.evaluate(el => el.value, contractTextElement);
